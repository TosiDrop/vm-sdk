...
import { getSanitizedAddress } from './getSanitizedAddress';
...
describe('getSanitizedAddress', () => {
  const testApiToken = 'test-token-123';
  const expectedUrl = `${VM_URL}/api.php?action=sanitize_address&address=stake_12345`;
  ...
  it('should call axios.get with the correctly constructed URL and headers', async () => {
    await getSanitizedAddress('stake_12345')
